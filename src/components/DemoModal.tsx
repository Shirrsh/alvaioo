...
const DemoModal: React.FC<DemoModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-fade-in-up animation-duration-300" role="dialog" aria-modal="true" aria-labelledby="demo-modal-title">
      <div className="bg-white dark:bg-dark-card rounded-xl shadow-2xl w-full max-w-lg h-[70vh] max-h-[600px] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center">
            ...
            <h3 id="demo-modal-title" className="text-lg font-semibold text-gray-800 dark:text-white">ALVAIO AI Demo</h3>
          </div>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" aria-label="Close AI Demo modal">
            <X size={24} />
          </button>
        </div>

        {/* Chat Area */}
        <div className="flex-grow p-4 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600" aria-live="polite">
          {messages.map(msg => (
            <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[70%] p-3 rounded-xl ${msg.sender === 'user' ? 'bg-brand-primary text-white rounded-br-none' : 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-none'}`}>
                {msg.sender === 'bot' && !msg.isLottie && <Bot size={16} className="inline mr-1 mb-0.5" aria-hidden="true" />}
                {msg.sender === 'user' && <User size={16} className="inline mr-1 mb-0.5" aria-hidden="true" />}
                {msg.isLottie ? (
            ...
            <div className="flex justify-start">
              <div className="max-w-[70%] p-3 rounded-lg bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-none">
                <div className="flex items-center space-x-1">
                    <Bot size={16} className="inline mr-1" aria-hidden="true" />
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center space-x-2">
            <input
              ...
              aria-label="Type your message to ALVA AI"
